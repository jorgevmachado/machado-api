...
def build_service(*, party, wild_pokemon):
    ...
    service.home_cache.delete_cache = AsyncMock()
...
async def test_from_session_and_has_active_battle_cover_factory_helpers():
    service = BattleSessionService.from_session(AsyncMock())
...
async def test_service_uses_repository_party_fallback_without_public_service():
    ...
    with patch(
        'app.domain.trainer.battle.service.TrainerPartyService.to_party_schema',
...
@pytest.mark.asyncio
async def test_get_active_battle_summary_returns_none_without_active_session():
    trainer = build_trainer()
    party = [build_party_member(trainer)]
    wild_pokemon = build_wild_pokemon()
    service, repository = build_service(party=party, wild_pokemon=wild_pokemon)
    ...
    result = await service.get_active_battle_summary_by_trainer_id(trainer.id)

    assert result is None


@pytest.mark.asyncio
async def test_get_active_battle_summary_returns_normalized_summary():
    trainer = build_trainer()
    party = [build_party_member(trainer)]
    wild_pokemon = build_wild_pokemon()
    service, repository = build_service(party=party, wild_pokemon=wild_pokemon)
    ...
    repository.active_session = session

    result = await service.get_active_battle_summary_by_trainer_id(trainer.id)

    assert result is not None
    assert result.battle_session_id == session.id
...
async def test_list_logs_falls_back_to_latest_terminal_session():
    ...
    repository.active_session = None
    repository.latest_session = session
...
@pytest.mark.asyncio
async def test_use_move_marks_battle_as_wild_pokemon_defeated_when_wild_faints():
    trainer = build_trainer()
...
async def test_process_wild_response_returns_early_for_inactive_status_and_fainted_member():
    ...
    session.status = BattleSessionStatusEnum.WILD_POKEMON_DEFEATED
    ...
    assert session.status == BattleSessionStatusEnum.TRAINER_DEFEATED


@pytest.mark.asyncio
async def test_flee_marks_battle_as_escaped():
    trainer = build_trainer()
    party = [build_party_member(trainer)]
    wild_pokemon = build_wild_pokemon()
    service, repository = build_service(party=party, wild_pokemon=wild_pokemon)
    ...
    assert result.status == BattleSessionStatusEnum.ESCAPED
    assert repository.session.commits == 1
    assert repository.logs[-1].payload["status"] == BattleSessionStatusEnum.ESCAPED.value
