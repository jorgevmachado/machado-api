...
def get_battle_log_service(session: Session) -> BattleLogService:
    return BattleLogService(BattleLogRepository(session))


Service = Annotated[BattleLogService, Depends(get_battle_log_service)]
CurrentUser = Annotated[User, Depends(get_current_user)]


def get_battle_log_filter(
    page: int | None = None,
...
@router.get(
    "",
    response_model=CustomLimitOffsetPage[BattleLogSchema] | list[BattleLogSchema],
    status_code=HTTPStatus.OK,
)
async def list_all(
    service: Service,
    current_user: CurrentUser,
    page_filter: Annotated[FilterPage, Depends(get_battle_log_filter)] = None,
):
...
@router.get("/{param}", response_model=BattleLogSchema, status_code=HTTPStatus.OK)
