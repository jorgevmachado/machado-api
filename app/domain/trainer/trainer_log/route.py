...
from app.domain.trainer.trainer_log.schema import TrainerLogSchema
...
def get_trainer_log_service(session: Session) -> TrainerLogService:
    return TrainerLogService(TrainerLogRepository(session))


Service = Annotated[TrainerLogService, Depends(get_trainer_log_service)]
CurrentUser = Annotated[User, Depends(get_current_user)]


def get_trainer_log_filter(
    page: int | None = None,
...
async def list_all(
    ...
    page_filter: Annotated[FilterPage, Depends(get_trainer_log_filter)] = None,
):
...
@router.get("/{param}", response_model=TrainerLogSchema, status_code=HTTPStatus.OK)
