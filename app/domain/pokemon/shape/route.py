...
def get_shape_service(session: Session) -> ShapeService:
    return ShapeService(ShapeRepository(session))


Service = Annotated[ShapeService, Depends(get_shape_service)]
CurrentUser = Annotated[User, Depends(get_current_user)]


def get_shape_filter(
    page: int | None = None,
...
@router.get(
    "",
    response_model=CustomLimitOffsetPage[ShapeSchema] | list[ShapeSchema],
    status_code=HTTPStatus.OK,
)
async def list_all(
    service: Service,
    current_user: CurrentUser,
    page_filter: Annotated[FilterPage, Depends(get_shape_filter)] = None,
):
...
@router.get("/{param}", response_model=ShapeSchema, status_code=HTTPStatus.OK)
