...
class OwnedPokemonMove:
    __tablename__ = "owned_pokemon_moves"

    owned_pokemon_id: Mapped[UUID] = mapped_column(
        ForeignKey("owned_pokemons.id"), nullable=False
    )
    pokemon_move_id: Mapped[UUID] = mapped_column(
        ForeignKey("moves.id"), nullable=False
    )
    ...
    pokemon_move: Mapped["Move"] = relationship(
        lazy=default_lazy,
        init=False,
        back_populates="owned_pokemon_moves",
    )
